...
For now longdouble is not supported as an array, pointer or custom type, 
but adding support would not be hard, so if this is something that you 
need, then please open a support ticket.
