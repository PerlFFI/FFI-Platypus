...
by your C compiler and by libffi.  Complex numbers are only supported
in very recent versions of libffi, and as of this writing the latest
production version doesn't work on x86_64, though it does appear to be
working with the latest version in git.

C<complex_float> and C<complex_double> are not (yet!) supported as return
types.  It would not be hard to implement, but there are peformance
ramifications depending on how the interface is implemented so I would
like to discuss it with someone who might actually use this feature.
If you need it, please open a support ticket on the 
L<project's github issue traacker|FFI::Platypus#SUPPORT>.
