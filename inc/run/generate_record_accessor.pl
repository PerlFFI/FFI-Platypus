...
my @list = map {
  (
    { ffi_type => "uint$_", c_type => "uint${_}_t", perl_type => "UV" },
    { ffi_type => "sint$_", c_type =>  "int${_}_t", perl_type => "IV" },
  )
} (8,16,32,64);

push @list, map { { ffi_type => $_, c_type => $_, perl_type => "NV" } } qw( float double );
