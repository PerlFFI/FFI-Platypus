...
subtest 'custom type input' => sub {
  plan tests => 2;
  custom_type uint16 => type1 => { perl_to_native => sub { is $_[0], 2; $_[0]*2 } };
...
subtest 'custom type output' => sub {
  plan tests => 2;
  custom_type uint16 => type2 => { native_to_perl => sub { is $_[0], 2; $_[0]*2 } };
...
subtest 'custom type post' => sub {
  plan tests => 2;
  custom_type uint16 => type3 => { perl_to_native_post => sub { is $_[0], 1 } };
